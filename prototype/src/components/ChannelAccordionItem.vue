<template>
  <section class="accordion-item" :class="{ open: open }">
    <button type="button" class="accordion-trigger" @click="emit('toggle')">
      <span class="badge" :style="{ background: meta.accent }" />
      <span class="name">{{ meta.name }}</span>
      <span class="status" :data-status="channel.status">{{ statusLabel }}</span>
...
import type { ChannelMeta, ChannelRuntime } from "../types/app.types";
...
.accordion-panel :deep(.mock-viewport) {
  border-radius: 0;
